...
| `lib/payment/e_shop_payment_bottom_sheet.dart` | 支付方式列表新增支付宝一行（仅设备已安装支付宝时显示，未安装时不显示任何内容）及 `_startAlipayProcess` |
...
→ POST /api/v1/alipay/createPharmacyOrder {orderId, currency, subject}
   后端：ExchangeService 把订单币种换算成人民币 → 建 Payment(type=ALIPAY) → 返回签名后的 orderString + outTradeNo
...
1. 设备安装支付宝，支付弹窗出现"支付宝"行；卸载后整行不出现，也没有提示文字。
