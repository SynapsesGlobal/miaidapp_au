...
class _EShopDetailsState extends State<EShopDetails> {
  ...
  void initState() {
    ...
    eShopDetailsStore.getProductCategories(
      widget.services.api,
  ...
  void _onScroll() {
    ...
    final triggerPoint = _scrollController.position.maxScrollExtent - 300;
    if (_scrollController.position.pixels >= triggerPoint &&
        eShopDetailsStore.has_more &&
        !eShopDetailsStore.isProductByCategoryLoading) {
  ...
  void _restoreScrollPosition() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (eShopDetailsStore.scrollPosition > 0 && _scrollController.hasClients) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Observer(builder: (context) {
        if (eShopDetailsStore.isLoading) {
        ...
        return CustomScrollView(
          ...
          slivers: [
            SliverToBoxAdapter(child: _pharmacyHeader(context)),
            ...
            SliverToBoxAdapter(
              child: Padding(
                padding: const EdgeInsets.only(left: 21, top: 4, bottom: 6),
                child: Text(
                  eShopDetailsStore.categoryName,
                  style: GoogleFonts.rubik(
                    color: AppColors.k010101,
                    letterSpacing: 0.37,
                    fontSize: 12,
            ...
            if (eShopDetailsStore.isProductByCategoryLoading && eShopDetailsStore.products.isEmpty)
              SliverFillRemaining(
                child: Center(child: progressIndicator()),
              )
            else
              _productsSliverList(context),
          ],
        );
      }),
    );
  ...
  Widget _productsSliverList(BuildContext context) {
    final showFooter = eShopDetailsStore.isProductByCategoryLoading && eShopDetailsStore.products.isNotEmpty;
    final itemCount = eShopDetailsStore.products.length + (showFooter ? 1 : 0);
    return SliverList(
      delegate: SliverChildBuilderDelegate(
        (context, index) {
          if (index == eShopDetailsStore.products.length) {
            return Padding(
          ...
          return _listItem(index, context, eShopDetailsStore.products[index]);
        },
                    ...
                    child: Column(
                      ...
                      children: [
                        Padding(
                          ...
                          child: Text(
                            widget.params?.pharmacyLocation.address.toString() ?? '',
                            style: GoogleFonts.rubik(
        ...
        child: TapDebouncer(
          onTap: () async {
            if (!eShopDetailsStore.products[index].isPrescriptionItem!) {
              await Navigator.push(context, MaterialPageRoute<void>(
                builder: (context) => getIt<ProductDetails>(
