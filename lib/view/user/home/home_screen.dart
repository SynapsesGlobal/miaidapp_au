    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          Padding(
            ...
            child: InkWell(
              child: Icon(Icons.qr_code, color: AppColors.k0cbcc5, size: isTablet ? 32 : 24,),
              onTap: () async {
                if (store.user.isLoggedIn) {
                  ...
                  // 扫码页可能返回三种值：Map(成功)、''(扫到非法码)、null(用户点返回)。
                  // 此前用 result.toString().isNotEmpty 判断，null 会被当成有效值再取下标导致崩溃。
                  if (result is Map) {
            ...
            child: InkWell(
              onTap: () {
                ...
                showCupertinoModalPopup(
                  context: context,
                  // barrier 不响应点击关闭：iPad 大屏上，右上角触发的点击落点在底部弹框之外的
                  // barrier 区域，会被 barrier 接收并立刻 dismiss，表现为「弹出即消失」。
                  // 弹框本身保留「取消」按钮，关闭体验不受影响。
                  barrierDismissible: false,
                  builder: (popupContext) {
                    // CupertinoActionSheet 横屏时按屏幕较短边(height)计算宽度，iPad 横屏下会变窄；
                    // 这里强制以竖屏方式计算，让弹框横向铺满屏宽。
                    final mq = MediaQuery.of(popupContext);
                    return MediaQuery(
                      data: mq.copyWith(size: Size(mq.size.width, mq.size.width + 1)),
                      child: action,
                    );
                  },
                );
      ...
      body: WithNotificationHandlerWidget(
        ...
        child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
          ...
          SizedBox(height: Utils.isPad(context) ? 12 : 0),
          // AI Banner
          Padding(
            padding: EdgeInsets.only(
              left: Utils.isPad(context) ? 16 : 0,
              right: Utils.isPad(context) ? 16 : 0,
              top: 8,
            ),
            child: TapDebouncer(
              onTap: () async {
              ...
              builder: (context, onTap) => InkWell(
                ...
                child: Stack(
                  children: [
                    Image.asset(
                      Utils.isPad(context) ? 'assets/banner_pad.png' : 'assets/banner.png',
                      width: double.infinity,
                      fit: BoxFit.cover,
                    ),
                    Positioned.fill(
                      child: Padding(
                        padding: EdgeInsets.only(left: Utils.isPad(context) ? 60 : 40, top: 14, bottom: 29, right: 130),
                        child: FittedBox(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                ...
                                style: GoogleFonts.rubik(
                                  color: Colors.white,
                                  fontSize: Utils.isPad(context) ? 22 : 16,
                                  fontWeight: FontWeight.bold,
                                ),
                                maxLines: 2,
                                overflow: TextOverflow.ellipsis,
                              ...
                              Text(
                                ...
                                style: GoogleFonts.rubik(
                                  ...
                                  fontSize: Utils.isPad(context) ? 18 : 13,
                                ),
                                maxLines: 2,
                                overflow: TextOverflow.ellipsis,
                              ),
          ...
          SizedBox(height: Utils.isPad(context) ? 20 : 0,),
