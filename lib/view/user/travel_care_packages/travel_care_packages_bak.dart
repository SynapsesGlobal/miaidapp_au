...
class _TravelCarePackagesState extends State<TravelCarePackages> {
    ...
    return Scaffold(
      ...
      body: Observer(
        builder: (context) => ListView(children: [
          ...
          if (widget.services.activeSubscriptionStore.activeCustomerSubscriptionDetail == null)
            _noActivePackage(context),
          SizedBox(
            height: 14,
          ),
          if (widget.services.activeSubscriptionStore.activeCustomerSubscriptionDetail != null)
            _activePackage(context),
          SizedBox(
            height: 28.4,
          ),
          Padding(
  ...
  Padding _activePackage(BuildContext context) {
    final activeSubscription = widget
        .services.activeSubscriptionStore.activeCustomerSubscriptionDetail;
  ...
  int _daysRemaining(SubscriptionDetail? activeSubscription) {
    if (activeSubscription?.companyCustomerSubscription != null) {
      return DateTime.parse(activeSubscription!.companyCustomerSubscription!.endAt!)
          .difference(DateTime.now())
          .inDays;
    } else if (activeSubscription?.customerSubscription != null) {
      var remainingDays = DateTime.parse(activeSubscription!.customerSubscription!.endAt!)
          .difference(DateTime.now())
          .inDays;

      if (activeSubscription.customerSubscription!.extension != null) {
        remainingDays = remainingDays + activeSubscription.customerSubscription!.extension!;
      }
